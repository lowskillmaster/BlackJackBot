...
class Game(object):
    ...
    def show_hand(self):
        player_hand = ''.join(self.player.hand)
        return f'Ваши карты: {player_hand}\nКарты Диллера: {self.dealer.hand}'

    def check_win(self):
        win_str = ""
        if self.player.count > 21:
            win_str += "☠️ Вы проиграли\n" + self.show_hand() + "\n"
        elif self.dealer.count > 21 >= self.player.count:
            win_str += "🥳 Вы победили!!!\n" + self.show_hand() + "\n"
